...
async fn command_list_ok_basic() {
    ...
    assert_eq!(list_ok_count, 2, "expected 2 list_OK separators, got: {resp}");
...
async fn command_list_error_stops_batch() {
    ...
    assert!(
        resp.contains("1@"),
        "ACK should reference index 1: {resp}"
    );
}
...
async fn command_list_ok_error_stops_batch() {
    ...
    let resp = client
        ...
        .await;
    assert!(resp.starts_with("ACK ") || resp.contains("ACK "), "batch error should return ACK: {resp}");
}
...
async fn command_list_end_without_begin() {
    ...
    assert!(resp.starts_with("ACK "), "end without begin should be an error: {resp}");
}
...
async fn command_list_preserves_order() {
    let (_server, mut client) = setup().await;
    let resp = client
        .command_list_ok(&["ping", "ping", "ping"])
        .await;
